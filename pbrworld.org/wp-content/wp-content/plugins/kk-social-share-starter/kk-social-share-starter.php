...
function sss_kk_execute() {
	...
	// new ver - from user feedback
	$twitter_button = '<a rel="nofollow" href="http://twitter.com/home?status='.htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8').'%20'.get_permalink().$twitter_name.'" target="_blank" title="Tweet this"><img src="'.plugins_url('_.png', __FILE__).'" class="sss_twitter_button" /></a> ';
	...
	if(is_array($f_image)) {
		// new ver - from user feedback
		$pin_button = '<a rel="nofollow" href="//www.pinterest.com/pin/create/button/?url='.get_permalink().'&media='.$f_image[0].'&description='.htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8').'" target="_blank" title="Share this on Pinterest"><img src="'.plugins_url('_.png', __FILE__).'" class="sss_pin_button" /></a> ';
